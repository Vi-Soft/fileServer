...
public final class EmailAddress
	...
	private static final long serialVersionUID = -6051982613738951029L;
	 ...
	 * allowing them.
	 * 
	 * If you don't want to allow them, and only want to allow valid domain
	 * names
	 * 
	 ...
	 * 
	 * (using quotes and angle brackets around the raw address) are allowed,
	 * e.g.:
	 *
	...
	private final boolean bouncing = true;
	 ...
	 * @param addr
	 */
	public EmailAddress(final EmailAddress addr) {
		this(addr.getEmailAddr(), addr.getLabel(), addr.isVerified());
	}

	/**
	 * The verified and labeled email address.
	 * 
	 * @param emailAddr
	 * @param label
	 * @param verified
	 */
	@BsonCreator
	public EmailAddress(@BsonProperty("emailAddr") final String emailAddr,
			@BsonProperty("label") final String label,
			@BsonProperty("verified") final boolean verified) {
		...
		if (isValidEmailAddr(emailAddr)) {
			...
			throw new EmailRuntimeException(
					"Invalid email address: " + emailAddr);
		}
		this.label = label;
	...
	/**
	 * The verified but labeled email address.
	 * 
	 * @param emailAddr
	 * @param label
	 */
	public EmailAddress(@BsonProperty("emailAddr") final String emailAddr,
			@BsonProperty("label") final String label) {
		this(emailAddr, label, false);
	}

	/**
	 * The unverified and unlabeled email address.
	 * 
	 * @param emailAddr
	 */
	public EmailAddress(@BsonProperty("emailAddr") final String emailAddr) {
		this(emailAddr, "undefined", false);
	}
	...
	public EmailAddress setEmailAddr(final String emailAddr) throws Exception {
		return ClassUtil.setter(this, "emailAddr", emailAddr);
	...
	public EmailAddress setBouncing(final boolean bouncing) throws Exception {
		return ClassUtil.setter(this, "bouncing", bouncing);
	...
	public EmailAddress setVerified(final boolean verified) {
		return ClassUtil.setter(this, "verified", verified);
	...
	public EmailAddress setLabel(final String label) throws Exception {
		return ClassUtil.setter(this, "label", label);

	}

	/**
	 * 
	 * Returns whether or not the emailAddr represented by this object instance
	 * is valid
	 * 
	 ...
	 * 
	 * 
	 * @return true if the emailAddr represented by this instance is valid
	 *         according
	 * 
	...
	public boolean isValid() {
		...
		return isValidEmailAddr(getEmailAddr());
	 ...
	 * 
	 * @return true if the given emailAddr valid according to RFC 2822, false
	 *         otherwise.
	 * 
	 */
	...
	public boolean equals(Object o) {
		...
		if (o instanceof EmailAddress) {
			...
			return getEmailAddr().equals(ea.getEmailAddr());
	...
	@Override
	public int compareTo(EmailAddress addr) {
		if(addr != null)
			return this.getEmailAddr().compareTo(addr.getEmailAddr());
		return 1;
	}
